...
# === Проверка прав ===
if [ "$EUID" -eq 0 ]; then
  echo -e "${RED}Не запускайте от root. Используйте sudo.${NC}"
  exit 1
fi
...
sudo apt update -qq
sudo apt install -y -qq curl wget git openssl postgresql postgresql-contrib nginx certbot python3-certbot-nginx 2>/dev/null || true
...
# === 4. Клонирование репозитория если ещё нет ===
...
# === 5. Создание .env и генерация секретов ===
...
# Создаём базу
sudo -u postgres psql -t -c "SELECT 1 FROM pg_database WHERE datname='${DB_NAME}'" | grep -q 1 || \
sudo -u postgres psql -c "CREATE DATABASE ${DB_NAME} OWNER ${DB_USER};"

sudo -u postgres psql -c "GRANT ALL PRIVILEGES ON DATABASE ${DB_NAME} TO ${DB_USER};" 2>/dev/null

# Сохраняем старый .env если есть
...
npm run seed 2>/dev/null || npx tsx scripts/seed.ts
...
# === 9. SSL / Nginx Reverse Proxy (если указан --domain) ===
if [ -n "$DOMAIN" ]; then
  echo -e "${YELLOW}[9/10] Настройка Nginx + SSL для ${DOMAIN}...${NC}"

  # Настройка Nginx
  sudo tee /etc/nginx/sites-available/authlauncher > /dev/null << NGINXEOF
server {
    listen 80;
    server_name ${DOMAIN};
...
NGINXEOF
  ...
  # Включаем сайт
  sudo ln -sf /etc/nginx/sites-available/authlauncher /etc/nginx/sites-enabled/
  ...
  echo -e "${YELLOW}  ⚠️  SSL не удался. Попробуйте вручную: sudo certbot --nginx -d ${DOMAIN}${NC}"
...
if curl -s -o /dev/null -w "%{http_code}" "http://localhost:${PORT}/api/v1/health" | grep -q "200"; then
  ...
  echo -e "${RED}  ❌ Сервер не отвечает. Проверьте: sudo journalctl -u authlauncher.service -f${NC}"
fi
...
echo -e "  ${YELLOW}chmod +x updater.sh && ./updater.sh --version ${VERSION}${NC}"
echo ""
echo -e "${CYAN}🗑️  Удаление:${NC}"
echo -e "  ${YELLOW}./installer.sh --remove${NC}"
echo -e "  ${YELLOW}ИЛИ: curl -O https://raw.githubusercontent.com/xotiks/auth-launcher/main/uninstall.sh && bash uninstall.sh${NC}"
echo ""
echo -e "${GREEN}============================================${NC}"